...
const variableWhiteListInDev = window.__MFE_DEVELOPMENT__ ? [
...
const unscopables = array2TruthyObject(
  lodashExports.without(cachedGlobals, ...accessingSpiedGlobals.concat(overwrittenGlobals))
);
const useNativeWindowForBindingsProps = /* @__PURE__ */ new Map([
  ["fetch", true],
  ["mockDomAPIInBlackList", false]
]);
...
function manualInvokeElementOnLoad(element) {
  ...
  if (lodashExports.isFunction(element.onload)) {
...
function manualInvokeElementOnError(element) {
  ...
  if (lodashExports.isFunction(element.onerror)) {
...
const execScriptsWithSandbox = (args, rawFn, mountDOM, proxy) => {
  ...
  if (src) {
    ...
    _execScripts(null, [src], proxy.proxy, {
      // fetch,
      strictGlobal,
      scopedGlobalVariables,
      beforeExec: () => {
                ...
                if (["replaceChild", "insertBefore", "appendChild"].includes(bodyProp)) {
                  return function(...args) {
                    if (args[0] && args[0].tagName === "SCRIPT" && !opts.disableScriptSandbox) {
                      return execScriptsWithSandbox(args, bodyTarget[bodyProp], bodyTarget, self2);
                    } else if (args[0] && args[0].classList) {
                      args[0].classList.add(self2.code);
                    }
              ...
              if (p === "addEventListener") {
                return function(...args) {
                  if (typeof args[1] === "function") {
                    args[1].__mfe_app_code = self2.code;
                  }
...
const proxyWin = getProxyWin('vite-vue-app', {});

export { proxyWin as p };
